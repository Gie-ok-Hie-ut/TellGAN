...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    elif which_model_netG == 'Tell_Generator':
        netG = Tell_Generator()
    else:
...
class Tell_Generator(torch.nn.Module):
     def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, gpu_ids=[], padding_type='reflect'):
        super(Tell_Generator, self).__init__()
        self.Tell_Encoder = Tell_Encoder(self, input_nc, output_nc,ngf)
        self.Tell_Decoder = Tell_Decoder(self, input_nc, output_nc,ngf)

    def forward(self, input):

        #Should get [image, next word] as an input.
        output_image = input #?
        output_transition = self.Tell_Decoder.forward(self.Tell_Encoder.forward(input))

        #Sum up with the original image
        output_result = torch.sum(output_image) + output_transition

        return output_result

    def getEncoder(self):
        return self.Tell_Encoder

    def getDecoder(self):
        return self.Tell_Decoder


class Tell_Encoder(torch.nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, gpu_ids=[], padding_type='reflect'):
        super(Tell_Encoder, self).__init__()

        #Only needs 1 Encoder
        self.ImgEncoder = ResnetGenerator_Encoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=3, gpu_ids=gpu_ids)

        self.WordEmbed = WordEmbed()
        self.ImgLSTM = ImgLSTM(input_nc, output_nc, ngf, norm_layer= norm_layer, use_dropout=use_dropout, gpu_ids = gpu_ids)


    def forward(self, input):
        output1 = self.Encoder.forward(input) # ok
        output2 = self.WordEmbed.forward(input) # no




        output_result = self.ImgLSTM.forward(output_concat) # Can this store the previous data? 

        return output_result

    def getImgEncoder(self):
        return self.ImgEncoder

    def getLSTM(self):
        return self.ImgLSTM

    def getWordEmbed(self):
        return self.WordEmbed
