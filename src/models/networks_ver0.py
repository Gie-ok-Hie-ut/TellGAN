...
class Tell_Generator(torch.nn.Module):
    ...
    def forward(self, input):
        return self.Tell_Decoder.forward(self.Tell_Encoder.forward(input))
...
class Tell_Encoder(torch.nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, gpu_ids=[], padding_type='reflect'):
        ...
        self.Encoder = ResnetGenerator_Encoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=3, gpu_ids=gpu_ids)
        self.WordEmbedding = WordEmbedding()
        self.ImgLSTM = ImgLSTM(input_nc, output_nc, ngf, norm_layer= norm_layer, use_dropout=use_dropout, gpu_ids = gpu_ids)


    def forward(self, input):
        output1 = self.Encoder(input) # ok
        output2 = self.WordEmbedding(input) # no
        output3 = self.ImgLSTM(input) # 

        #Each dimension should be changed to merge into one.
        output_concat = output1 + output2 + output3
